								  ...
								  if(error == '')
										  {
											  ...
											  $.ajax({ 
													...
													success: function(data){
															...
															if(doc_no!=''){
																// Check if this is "For Release" (doc_class2 == 2) and "Regional Office"
																var doc_class = $('input[name="doc_class2"]:checked').val();
																var recipient = $('#sender2').val();
																
																if(doc_class == 2 && recipient == 'Henry A. Adornado, Ph.D'){
																	// Send notification to Records Unit
																	$.ajax({
																		type: "POST",
																		url: "<?php echo base_url();?>c_dts/notify_records_unit",
																		data: {
																			doc_no: doc_no,
																			doc_class: doc_class,
																			recipient: recipient,
																			subject: $('#doc_subject2').val()
																		},
																		success: function(notif_response){
																			console.log('Notification sent to Records Unit');
																		},
																		error: function(){
																			console.log('Notification could not be sent, but document was created');
																		}
																	});
																}
